...
import SectionRenderer from '@/components/media-kit-editor/SectionRenderer';
...
export default function MediaKitEditor() {
  ...
  const mediaKitPreviewData = useMemo((): EditorPreviewData | null => {
    ...
    // Fix the type issue using a type assertion
    const getBaseProp = <K extends keyof EditorPreviewData>(
      ...
      if (template && basePreviewData && propName in basePreviewData) {
        // Use type assertion that preserves the specific field type
        return basePreviewData[propName] as EditorPreviewData[K];
      }
      return undefined;
    };
  ...
  }, [profile, formData, videoLinks, sectionVisibility, selectedTemplateId, stats]); 
  ...
  if (profileError || mediaKitError) {
    ...
    // Use more explicit type handling
    if (profileError) {
      if (typeof profileError === 'string') {
        errorMessage = profileError;
      } else if (profileError && typeof profileError === 'object' && 'message' in profileError) {
        // Cast to Record to avoid the 'never' type issue
        const errorObj = profileError as Record<string, unknown>;
        if (typeof errorObj.message === 'string') {
          errorMessage = errorObj.message;
        }
      }
    } else if (mediaKitError) {
      if (typeof mediaKitError === 'string') {
        errorMessage = mediaKitError;
      } else if (mediaKitError && typeof mediaKitError === 'object' && 'message' in mediaKitError) {
        // Cast to Record to avoid the 'never' type issue
        const errorObj = mediaKitError as Record<string, unknown>;
        if (typeof errorObj.message === 'string') {
          errorMessage = errorObj.message;
        }
      }
            ...
            <Tabs defaultValue="branding" className="w-full" onValueChange={setActiveTab}>
              ...
              <TabsContent value="content" className="space-y-4">
                {SECTIONS.filter(sec => sec.tab === 'content').map(section => {
                  // Create props for the SectionRenderer
                  ...
                  return (
                    <SectionRenderer
                      key={section.key}
                      section={section}
                      formComponentMap={formComponentMap}
                      sectionVisibility={sectionVisibility}
                      onVisibilityChange={handleVisibilityChange}
                      formProps={formProps}
                      switchTrackClass={switchTrackClass}
                      switchThumbClass={switchThumbClass}
                    />
                  );
                })}
              </TabsContent>
