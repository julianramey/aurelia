      ...
      <div className="container mx-auto max-w-5xl relative">
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-10">
            <Button className="bg-glow-purple hover:bg-glow-purple/90 text-white px-8 py-6 text-lg rounded-full">
              Create Your Media Kit
            </Button>
            <Button variant="outline" className="border-glow-purple/20 hover:border-glow-purple text-foreground px-8 py-6 text-lg rounded-full">
              See How It Works
            </Button>
          </div>

          <div className="flex items-center space-x-4 text-sm text-muted-foreground mt-10">
            <div className="flex -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-8 h-8 rounded-full bg-glow-soft-purple border-2 border-white" />
              ))}
            </div>
            <p>Loved by <span className="font-medium text-foreground">2,000+</span> micro-influencers</p>
          </div>
        </div>
        ...
        <div className="relative h-[500px] max-w-4xl mx-auto">
          <div className="absolute inset-0 rounded-2xl overflow-hidden shadow-sm border border-border/20">
            <img 
              src="https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1287&q=80" 
              alt="Glowfolio Media Kit Preview" 
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/30"></div>
            <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
              <h3 className="font-display text-xl font-medium">Your Media Kit</h3>
